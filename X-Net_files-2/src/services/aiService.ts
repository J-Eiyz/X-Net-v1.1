...
export const aiService = {
  ...
  sendMessage: async (message: string, isOnline: boolean): Promise<AIMessage> => {
    ...
    try {
      // Use JDoodle proxy server to send request to OpenAI API
      const response = await fetch('https://hooks.jdoodle.net/proxy', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          payload: {
            ...
            messages: [
              {
                role: "system",
                content: "You are X-Net AI, an expert assistant for Beyblade X enthusiasts. You have deep knowledge about Beyblade X products, strategies, tournaments, and optimal combinations. Your responses should be helpful, accurate, and reflect expertise in the Beyblade world. Keep responses under 150 words and focus specifically on answering the user's question about Beyblade X."
              },
              {
                role: "user",
                content: message
      ...
      // Handle the response structure from the proxy
